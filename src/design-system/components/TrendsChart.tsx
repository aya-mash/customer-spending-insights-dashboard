...
const CustomTooltip = ({ active, payload, label, data }: CustomTooltipProps) => {
  ...
  const diff = prevVal === undefined ? 0 : value - prevVal;
  const diffPercent = prevVal === undefined ? 0 : ((diff / prevVal) * 100);
      ...
      {prevVal !== undefined && (
        <div
          style={{
            ...
            fontSize: '13px',
            color: (() => {
              if (isPositive) return 'var(--color-error)';
              if (isNegative) return 'var(--color-success)';
...
interface TrendsChartProps {
  readonly data: MonthlyTrendPoint[];
}
...
export default function TrendsChart({ data }: TrendsChartProps) {
  ...
  if (data.length >= 2) {
    const lastValue = data.at(-1)!.totalSpent;
    ...
    <div className="trends-chart" aria-describedby={summaryId} aria-label="Monthly spending trends area chart">
      <ResponsiveContainer key={`chart-${themeKey}`} width="100%" height={340}>
        ...
        >
          ...
          <Tooltip content={<CustomTooltip data={data} />} cursor={{ stroke: CHART_COLORS.grid, strokeWidth: 1 }} />
